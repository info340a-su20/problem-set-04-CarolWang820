...
function huskiesLost(game) {
  if (game.home === 'UW' && game.opponent_score > game.home_score) {
    return true;
...
let lostG = huskyGames2016.filter(huskiesLost);
console.log(lostG);

//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!
console.log(huskyGames2016.filter(huskiesLost).map(extractOpponent));
...
function mostYardsPassing(game1, game2) {
  if (game1.passing_yards == 'undefined' || game1.passing_yards < game2.passing_yards) {
    return game2;
  } else {
    return game1;
  }
...
function makeCombinedFilter(a1, a2) {
  function combinedFilter(game) {
    return a1(game) && a2(game);
  }
